 ...
 * Created by qqzeng.
 */
public class BloomServerHandler extends ChannelInboundHandlerAdapter {
    private static final Logger LOGGER = LoggerFactory.getLogger(BloomServerHandler.class);

    private Map<String, Object> handlerMap;

    public BloomServerHandler(Map<String, Object> handlerMap) {
    ...
    public void channelRead(ChannelHandlerContext channelHandlerContext,
        ...
        if (!(object instanceof BloomRequest)) {
            bloomResponse.setException(new Exception("Error request!"));
        }
        BloomRequest bloomRequest = (BloomRequest) object;
...
//        channelHandlerContext.writeAndFlush(bloomResponse).addListener(ChannelFutureListener.CLOSE);
        ...
        LOGGER.debug("Server send response over at " + new Date());
    }
    ...
    private Object handleRequest(BloomRequest bloomRequest) {
        ...
        LOGGER.debug("Service bean has been found!" + new Date());
